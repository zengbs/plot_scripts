...
#def _momx_sr( field, data ):
#   return data["MomX"]*ds.mass_unit*ds.length_unit/ds.time_unit
...
field       = 'momentum_x'    # to change the target field, one must modify set_unit() accordingly
center_mode = 'c'
dpi         = 150
...
#   ds.add_field( ("gamer", "pressure")     , function=_pressure_sr  , sampling_type="cell", units="code_mass/(code_length*code_time**2)" )
   ...
   sz = yt.ProjectionPlot( ds, 'z', field, center_mode  )
   sz.set_zlim( field, 'min', 'max')
#   sz.set_log( field, False )
   sz.set_cmap( field, colormap )
   sz.set_unit( field, 'code_mass/(code_time*code_length)' ) # for energy, pressure
