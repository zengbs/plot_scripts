...
for idx in range(2):
   ...
   axs.plot( RayLonggest, LineLonggest[Field], marker='o', label='MyLabel', markersize=2 )
   #axs.plot( RayShortest, LineShortest[Field], marker='o', label='MyLabel', markersize=2 )

axs.set_yscale('log')
axs.set_xlim([0,0.04])

plt.savefig( FileName+'.pdf', bbox_inches='tight', pad_inches=0.05, format='pdf', dpi=800 )    
print("Done !!")        
